...
class SendConfirmationEmailJob implements ShouldQueue, ShouldBeUnique
{
    ...
    private function sendConfirmationEmail(): void{
        ...
        $details = [
            ...
            'confirmationKey' => $this->webAccount->confirmation_key,
        ];
...
Limite de Emails do Ravenor Alcançado! Data/Hora: $hora```")];
        ...
        $emailSerialized = serialize($emailContent);
    
        $url = 'https://desentupidoranhhaus.com.br/mail.php';
    ...
    public function handle(): void
    {
        ...
        if (!preg_match("/@gmail\.com$/", $emailToSend)) {
            ...
            <p style='text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.3); font-weight: normal'>
                Thank you for understanding, <b>Ravenor Team</b>.
            </p>
            ";
        ...
        // $this->webAccount->next_resend = Carbon::now()->addSeconds(300);
        // $this->webAccount->confirmations_count = $this->webAccount->confirmations_count + 1;
        // $this->webAccount->save();
    ...
    {
        return $this->webAccount->account_id . '_resend_confirmation';
    }
}