...
function LoadLoginBox() {
  if (loginStatus === "false") {
    document.getElementById('PlayNowContainer').childNodes[0].childNodes[1].src = JS_DIR_IMAGES
...
function LoginstatusTextAction(source) {
  if (loginStatus == "false") {
    window.location = JS_DIR_ACCOUNT + "/create";
  } else {
 ...
 * news=1&abouttibia=0&gameguides=0&library=0&community=0&account=0&support=0&
 * 
...
function LoadMenu() {
  ...
  if (self.name.lastIndexOf("&") == -1) {
    self.name = "news=1&library=0&community=0&account=0&team=0&shops=0&";
  }
  FillMenuArray();
  InitializeMenu();
}
...
function OpenMenuItem(sourceId) {
  ...
  $('#' + sourceId + '_Submenu').css({
    'visibility': 'visible',
    'display': 'block' 
  });

  $('#' + sourceId + '_Lights').css('visibility', 'hidden');
  $('#' + sourceId + '_Extend').css('background-image', 'url(' + JS_DIR_IMAGES + 'general/minus.gif)');
}
...
function CloseMenuItem(sourceId) {
  ...
  $('#' + sourceId + '_Lights').css('visibility', 'visible');
  $('#' + sourceId + '_Extend').css('background-image', 'url(' + JS_DIR_IMAGES + 'general/plus.gif)');
  $('#' + sourceId + '_Submenu').css({
    'visibility': 'hidden',
    'display': 'none'
  });
