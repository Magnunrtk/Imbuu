...
{
  // if it was clicked on the same row there is nothing to do
...
function BuildHelperDivLink(a_DivID, a_IndicatorDivContent, a_Title, a_Text, a_SubTopic) {
  var l_Qutput = '';
  l_Qutput += '<a href="#' + a_SubTopic + '" target="_blank" ><span class="HelperDivIndicator" onMouseOver="ActivateHelperDiv($(this), \'' + a_Title + '\', \'' + a_Text + '\', \'' + a_DivID + '\');" onMouseOut="$(\'#HelperDivContainer\').hide();" >' + a_IndicatorDivContent + '</span></a>';
  return l_Qutput;
}
...
function ActivateHelperDiv(a_Object, a_Title, a_Text, a_HelperDivPositionID, a_ServerName) {
  var l_Left = 0;
  var l_Top = 0;
  var l_WindowHeight = $(window).height();
  var l_PageHeight = $(document).height();
  var l_ScrollTop = $(document).scrollTop();
  if (a_ServerName !== undefined) {
    $('#HelperDivHeadline').html(a_Title.replaceAll('ServerName', a_ServerName));
    $('#HelperDivText').html(a_Text.replaceAll('ServerName', a_ServerName));
  } else {
...
function CopyContentOfFormInput(a_FormInputID) {
