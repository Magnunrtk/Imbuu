...
{
	TextMessage tmp = message; // Faz uma cópia modificável

	if ((int)tmp.type < 18 || (int)tmp.type > 27)
		tmp.type = MESSAGE_STATUS_CONSOLE_RED;

	NetworkMessage msg;
	msg.addByte(0xB4);
	msg.addByte(tmp.type);
	msg.addString(tmp.text);
	writeToOutputBuffer(msg);
}
